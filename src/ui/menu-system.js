// === ENHANCED MENU SYSTEM WITH TIME OF DAY ===
// File: src/ui/menu-system.js
// COMPLETE REPLACEMENT - Now includes time of day controls
...
class MenuSystem {
        ...
        this.menus = {
            main: {
                ...
                items: [
                    ...
                    { text: "Time of Day", action: () => this.showMenu('timeOfDay') }, // NEW
            ...
            // NEW: Time of Day menu
            timeOfDay: {
                title: "Time of Day Controls",
                items: [
                    { text: "← Back", action: () => this.goBack() },
                    { text: "Advance Time (N)", action: () => this.advanceTime() },
                    { text: "Set to Day (F1)", action: () => this.setTimeOfDay('day') },
                    { text: "Set to Dusk (F2)", action: () => this.setTimeOfDay('dusk') },
                    { text: "Set to Night (F3)", action: () => this.setTimeOfDay('night') },
                    { text: "Set to Dawn (F4)", action: () => this.setTimeOfDay('dawn') },
                    { text: "Show Time Status (J)", action: () => this.showTimeStatus() }
                ]
            },
            world: {
            ...
            help: {
                ...
                items: [
                    ...
                    { text: "Time Controls: N, F1-F4, J", action: null }, // NEW
                    ...
                    { text: "Day/Night affects vision range!", action: null } // NEW
                ]
            }
        };
    ...
    // NEW: Time of Day menu actions
    
    advanceTime() {
        const newTime = this.gameEngine.terrainSystem.advanceTimeOfDay();
        if (newTime) {
            const status = this.gameEngine.terrainSystem.getTimeOfDayStatus();
            this.gameEngine.uiController.showMessage(
                `Time: ${newTime.toUpperCase()} - ${status.description}`, 
                3000
            );
    ...
    setTimeOfDay(timeOfDay) {
        const success = this.gameEngine.terrainSystem.setTimeOfDay(timeOfDay);
        if (success) {
            const status = this.gameEngine.terrainSystem.getTimeOfDayStatus();
            this.gameEngine.uiController.showMessage(
                `Time: ${timeOfDay.toUpperCase()} - ${status.description}`, 
                2500
            );
            this.gameEngine.render();
        }
    ...
    showTimeStatus() {
        const status = this.gameEngine.terrainSystem.getFogOfWarStatus();
        const message = `${status.timeOfDay.toUpperCase()}: ${status.timeDescription} | Vision: ${status.visionRadius}+${status.forwardVisionRange} (${status.coneAngle}°)`;
        this.gameEngine.uiController.showMessage(message, 4000);
    }
    
    // Configuration menu actions (existing)
    ...
    showCurrentConfig() {
        ...
        const timeStatus = this.gameEngine.terrainSystem.getTimeOfDayStatus();
        
        const message = `World: ${worldConfig.minX} to ${worldConfig.maxX} | Deer: ${deerConfig.maxDeerCount} max | Time: ${timeStatus.timeOfDay}`;
        this.gameEngine.uiController.showMessage(message, 4000);
    }
   ...
   render(gameArea, getTerrainFunction, playerX, playerY) {
       ...
       for (let y = 0; y < menuHeight; y++) {
           ...
           for (let x = 0; x < menuWidth; x++) {
               ...
               let symbol = ' ';
               let className = 'symbol terrain-hills'; // Brown background
               ...
               if (y === 0 || y === menuHeight - 1) {
                   ...
                   className = 'symbol terrain-stone'; // Gray border
               ...
               // Draw title
                   ...
                   if (titleIndex >= 0 && titleIndex < this.currentMenu.title.length) {
                       ...
                       className = 'symbol terrain-village'; // Golden title
                   }
               }
               // Draw menu items
                   ...
                   if (x === 2 && isSelected) {
                       ...
                       className = isSelected ? 'symbol player' : 
                                  (item.action ? 'symbol terrain-village' : 'symbol terrain-stone'); // Red if selected, brown if clickable, gray if info
                   }
               }
       ...
       if (instructionY < lines.length) {
           ...
           for (let i = 0; i < instruction.length; i++) {
               ...
               if (charIndex >= 0) {
                   this.setCharAt(lineChars, charIndex, instruction[i], 'symbol terrain-village');
               }
           }
   ...
   parseLineToChars(line) {
       ...
       while ((match = spanRegex.exec(line)) !== null) {
           ...
           for (let i = lastIndex; i < match.index; i++) {
               chars.push({ symbol: ' ', className: 'symbol terrain-grass' });
           }
   ...
   setCharAt(lineChars, index, symbol, className) {
       ...
       while (lineChars.length <= index) {
           lineChars.push({ symbol: ' ', className: 'symbol terrain-grass' });
       }
