// === ENHANCED INPUT HANDLER WITH COMPANION CONTROLS ===
// File: src/systems/input-handler.js
// COMPLETE REPLACEMENT - Now includes companion dog controls
...
class InputHandler {
    ...
    setupKeyboardInput() {
        document.addEventListener('keydown', (e) => {
            // Check if menu is visible and handle menu input first
    ...
    handleSpecialInput(e) {
        ...
        // === NEW: COMPANION DOG CONTROLS ===
        
        // Call companion (C key)
        if (e.key === 'c' || e.key === 'C') {
            e.preventDefault();
            if (this.game.terrainSystem.companionSystem) {
                this.game.terrainSystem.companionSystem.callCompanion();
                this.game.uiController.showMessage('Come here, buddy! ♥', 1500);
            } else {
                this.game.uiController.showMessage('No companion available', 1000);
            }
            return true;
        }
        
        // Show companion status (P key for "Pet")
        if (e.key === 'p' || e.key === 'P') {
            e.preventDefault();
            if (this.game.terrainSystem.companionSystem) {
                const debug = this.game.terrainSystem.companionSystem.getDebugInfo();
                if (debug) {
                    const message = `Companion: ${debug.state} at (${debug.position.x}, ${debug.position.y}) | Distance: ${debug.distanceToPlayer}`;
                    this.game.uiController.showMessage(message, 3000);
                } else {
                    this.game.uiController.showMessage('No companion found', 1000);
                }
            }
            return true;
        }
        ...
        if (e.key === 'n' || e.key === 'N') {
            e.preventDefault();
            const newTime = this.game.terrainSystem.advanceTimeOfDay();
            if (newTime) {
                const status = this.game.terrainSystem.getTimeOfDayStatus();
        ...
        // Quick time set controls
        if (e.key === 'F1') {
            e.preventDefault();
            this.game.terrainSystem.setTimeOfDay('day');
            this.game.uiController.showMessage('Time set to DAY - Bright daylight, excellent visibility', 2500);
            this.game.render();
            return true;
        }
        ...
        if (e.key === 'F2') {
            e.preventDefault();
            this.game.terrainSystem.setTimeOfDay('dusk');
            this.game.uiController.showMessage('Time set to DUSK - Twilight, vision starting to fade', 2500);
            this.game.render();
            return true;
        }
        ...
        if (e.key === 'F3') {
            e.preventDefault();
            this.game.terrainSystem.setTimeOfDay('night');
            this.game.uiController.showMessage('Time set to NIGHT - Dark, limited torch-like vision', 2500);
            this.game.render();
            return true;
        }
        ...
        if (e.key === 'F4') {
            e.preventDefault();
            this.game.terrainSystem.setTimeOfDay('dawn');
            this.game.uiController.showMessage('Time set to DAWN - Early morning, vision improving', 2500);
            this.game.render();
            return true;
        }
        
        // Show time/vision status
        if (e.key === 'j' || e.key === 'J') {
            e.preventDefault();
    ...
    handleUndoRedoInput(e) {
        ...
        // Undo (Ctrl+Z, but not if it conflicts with other controls)
        if (e.key === 'z' && !e.shiftKey) {
            if (!['e', 'w', 'g', 's', 'r', 'l', 'k'].includes(e.key)) {
                e.preventDefault();
                this.game.undo();
                return true;
            }
